...
export async function createAuthApp(config: AuthConfig, deps: AuthAppDependencies): Promise<FastifyInstance> {
  ...
  app.addHook("onResponse", async (request, reply) => {
    ...
    metrics.inc("http_requests_total", { service: "auth", method: request.method, route, status: reply.statusCode });
    metrics.observe("http_request_duration_ms", duration, { service: "auth", method: request.method, route });
  });
  ...
  await registerAuthRoutes(app, config, deps);
  return app;
}
