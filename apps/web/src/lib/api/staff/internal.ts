// ════════════════════════════════════════════════════════════════════════════
// internal.ts — Shared gateway utilities for staff API clients
// Service : web → gateway (port 4000) → core / billing services
// Exports : callGateway, isUnauthorized, toGatewayError, withAuthorizedSession
// ════════════════════════════════════════════════════════════════════════════
    ...
    return {
      ...
      payload: {
        ...
        error: {
          code: "NETWORK_ERROR",
          message:
            "Unable to reach the API. Confirm the gateway is running and NEXT_PUBLIC_GATEWAY_BASE_URL is correct."
        }
      }
    };
  }
...
export function isUnauthorized<T>(response: RawGatewayResponse<T>): boolean {
  return (
    response.status === 401 ||
    response.payload.error?.code === "UPSTREAM_UNAUTHORIZED"
  );
}
...
// Retries once with a refreshed token on 401; returns error on second failure.
export async function withAuthorizedSession<T>(
  session: AuthSession,
  runner: (accessToken: string) => Promise<{
  ...
  if (second.unauthorized) {
    return {
      ...
      error: toGatewayError(
        ...
        "Session is valid but authorization failed for staff resources."
      )
    };
  }
