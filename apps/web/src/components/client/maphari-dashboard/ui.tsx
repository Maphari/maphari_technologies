 ...
 *
 * Ic — SVG icon component (maps icon names to stroke paths)
 * Av — Avatar circle with gradient/color background
 * Toggle — Simple toggle switch
 * ToggleRow, ActivityRow, TimeEntry — shared foundation, pre-bound to client styles
 * NavIcon — extracted to nav-icon.tsx
 */
import { styles } from "./style";

// ── Icon paths (merged from all dashboard screens) ────────────────────────
...
export function Av({
  ...
  const altTone = Boolean(gradient || color);
  return (
    <div className={`${styles.uiAvatar} ${sizeClass} ${altTone ? styles.uiAvatarAlt : ""}`}>
      {initials}
    </div>
  );
}
...
export function Toggle({ on, toggle }: { on: boolean; toggle: () => void }) {
  return (
    <button
      ...
      onClick={toggle}
      className={`${styles.uiToggle} ${on ? styles.uiToggleOn : ""}`}
    >
      <div className={`${styles.uiToggleKnob} ${on ? styles.uiToggleKnobOn : ""}`} />
    </button>
  );
}

// ── Shared UI components, pre-bound to client styles ──────────────────────
